...
@implementation crossfitMobileObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_crossfitmobile);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any crossfitMobile patches in JS before a context is loaded... nice for 
...
{
	// create crossfitMobile global object
	NSString *basePath = (url==nil) ? [[NSBundle mainBundle] resourcePath] : [[url path] stringByDeletingLastPathComponent];
	_crossfitmobile = [[crossfitMobileObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_crossfitmobile];
	
	NSString *_crossfitmobileNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithUTF8CString([_crossfitmobileNS UTF8String]);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_crossfitmobile valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.crossfitmobile.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
